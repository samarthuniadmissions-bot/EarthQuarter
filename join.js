...
function parseTimeFromMessage(message) {
  ...
  if (rangeMatch) {
    ...
    if (minutes > 59) {
      return { valid: false, error: "The start time in your message looks wrong. Use a time like 7 AM, 7:00 AM, or 19:00." };
    }
...
async function sendFormSubmitDetails(submission) {
  ...
  if (!response.ok) {
    throw new Error("FormSubmit could not send the details. Please try again.");
  }
