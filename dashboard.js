...
dashboardLead.textContent = planStart.display
  ? `Your recurring Earthquarter reminder is set for ${planStart.display}.`
...
calendarLink.href = app.buildRecurringCalendarLink({
  ...
  time: planStart.value,
  displayTime: planStart.display,
...
dashboardEvidenceForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    await app.prepareEvidencePhotoForEmail(dashboardEvidenceForm, file);
